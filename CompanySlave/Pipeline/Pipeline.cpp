...
#pragma region//ピクセルシェーダ読み込みとコンパイル
	//ピクセルシェーダ読み込みとコンパイル
	result = D3DCompileFromFile(
...
Pipeline::PipelineSet Pipeline::ParticleCreateGraphicsPipeline(ID3D12Device *dev)
{
	HRESULT result = S_FALSE;
	ComPtr<ID3DBlob> vsBlob; // 頂点シェーダオブジェクト
...
Pipeline::PipelineSet Pipeline::ParticleCreateGraphicsNoAplaha(ID3D12Device* dev)
{
	HRESULT result = S_FALSE;
	ComPtr<ID3DBlob> vsBlob; // 頂点シェーダオブジェクト
	ComPtr<ID3DBlob>gsBlod;//ジオメトリシェーダオブジェクト
